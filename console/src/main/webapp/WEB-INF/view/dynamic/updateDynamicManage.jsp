		...
		<div class="page-content">
			<div class="page-bar" style="margin-top: 40px;">
				<ul class="page-breadcrumb">
					...
					<li>
						<a href="<%=request.getContextPath()%>/dynamic/dynamicManage/${table.tableName}/update/${data.id}">${table.remark}修改</a>
					</li>
				</ul>
			</div>
			<!-- BEGIN FROM -->
					...
					<div class="portlet light bordered">
						<div class="portlet-title">
							<div class="caption font-green-haze">
								...
								<span class="caption-subject bold uppercase">${table.remark}修改</span>
							</div>
						</div>
						<div class="portlet-body form">
							<form id="form" role="form" class="form-horizontal" action="<%=request.getContextPath()%>/dynamic/dynamicManage/${table.tableName}/update" method="POST">
...
$(function() {
	...
	var obj = new Object();
	<c:forEach items="${columns}" var="column">
		if("${column.isAllowUpdate}"=='true'){
			<c:forEach items="${data}" var="d" >
				<c:if test="${d.key == column.columnName}">
					obj['${d.key}'] = '${d.value}';
				</c:if>
			</c:forEach>
		}
